...
namespace LH
{
    public partial class Client_Advocate : Form
    {
        utilities util = new utilities();
        String status_1 = utilities.status;
        
        public Client_Advocate()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    String insert = util.iud("INSERT INTO all_table (pid,f1, f2,f3,f4, status ) values ('0','" + f1.Text + "','" +f2.Text + "','" +f3.Text + "','" +f4.Text + "','" + status.Text + "')");
                    if (insert == "sucess")
                    {
                ...
                {
                    // Fixed: Removed the extra comma before 'where'
                    String insert = util.iud(
                        "update all_table set f1='" + f1.Text + "', f2='" + f2.Text + "', f3='" + f3.Text + "', f4='" + f4.Text + "' where id=" + lb1_id.Text
                    );
                    if (insert == "sucess")
                    {
        ...
        private void load_grid()
        {
            ...
            {
                sno=sno + 1;
                grd.Rows.Add(drow["id"],sno, drow[2], drow[3], drow[4], drow[5]);
            }
        }
        ...
        private void grd_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
           ...
           f4.Text = grd.SelectedRows[0].Cells[5].Value.ToString();
